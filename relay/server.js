...
<style>:root{color-scheme:dark}body{margin:0;min-height:100vh;display:grid;place-items:center;background:#06111d;color:#edf7ff;font:16px Arial,sans-serif}main{width:min(620px,calc(100% - 36px));padding:30px;border:1px solid #246382;border-radius:16px;background:#0b2236}h1{margin:0 0 12px;color:#28d7f5}.state{margin-top:22px;padding:14px 16px;border-left:4px solid #55dfa4;background:#071927;color:#55dfa4;font-weight:bold}.small{font-size:13px;color:#8daabd}.start{display:inline-block;margin-top:18px;padding:14px 18px;border-radius:8px;background:#28d7f5;color:#021019;text-decoration:none;font-weight:bold}</style></head><body><main><h1>Foorys MyE2iV3</h1><p>Łączę bezpośrednio z aktywną sesją MyE2i na wybranym dekoderze.</p><div id="state" class="state">Czekam na ekran MyE2i na dekoderze…</div><p class="small">Gdy przycisk się pojawi, kliknij go. Oficjalne MyE2iV3 samo przekaże wynik do tego tunelu.</p><script>const token=${safeToken},state=document.getElementById("state");function esc(v){const e=document.createElement("a");e.textContent=v;return e.innerHTML}async function check(){try{const r=await fetch("/v1/e2i/tunnel/"+encodeURIComponent(token)+"/status",{cache:"no-store"}),d=await r.json();if(d.ready&&d.targetUrl){state.innerHTML='<a class="start" href="'+esc(d.targetUrl)+'">Otwórz weryfikację MyE2iV3</a>';return}if(d.error){state.textContent=d.error;return}state.textContent=d.status==="failed"?"Nie udało się przygotować MyE2i.":"Czekam na aktywną sesję MyE2i na dekoderze…"}catch(_){state.textContent="Brak połączenia z Relay — ponawiam…"}setTimeout(check,2000)}check()</script></main></body></html>`;
}
